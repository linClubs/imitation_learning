...
def inference_process(args, config, ros_operator, policy, next_actions, stats, t, pre_action):
    ...
    while True and not rospy.is_shutdown():
        ...
        #all_actions = torch.tensor([0.0]*14)
    ...
    with torch.inference_mode():
        while True and not rospy.is_shutdown():
            ...
            if config['temporal_agg']:
                if args.use_robot_base:
                    all_time_actions = np.zeros([max_publish_step, max_publish_step + chunk_size, 16])
                else:
            ...
            while t < max_publish_step and not rospy.is_shutdown():
                ...
                ## 动作过滤
                if args.use_action_protection:

                    ## 左臂限制
                    # 对应的最小值限制
                    left_min_values = np.array([-1.1, -0.01, -0.01, -1.40, -1.14, -2.0, 0.0])
                    # 对应的最大值限制
                    left_max_values = np.array([0.45, 2.35, 3.1, 1.40, 1.3, 1.35, 20.0])
                    # 应用限制
                    left_action = np.maximum(left_min_values, np.minimum(left_action, left_max_values))

                    ## 右臂限制
                    # 对应的最小值限制
                    right_min_values = np.array([-0.5, 0.0, 0.01, -1.40, -1.59, -1.41, 0.0])
                    # 对应的最大值限制
                    right_max_values = np.array([1.1, 2.35, 2.8, 1.40, 1.56, 2.0, 20.0])
                    # 应用限制
                    right_action = np.maximum(right_min_values, np.minimum(right_action, right_max_values))

                    # 比较两个数组中的元素
                    equal_elements_left_min = np.equal(left_min_values, left_action)
                    equal_elements_left_max = np.equal(left_max_values, left_action)
                    equal_elements_right_min = np.equal(right_min_values, right_action)
                    equal_elements_right_max = np.equal(right_max_values, right_action)
                    # 检查是否有对应位置的相等元素
                    has_matching_elements_left_min = np.any(equal_elements_left_min)
                    has_matching_elements_left_max = np.any(equal_elements_left_max)
                    has_matching_elements_right_min = np.any(equal_elements_right_min)
                    has_matching_elements_right_max = np.any(equal_elements_right_max)
                    if has_matching_elements_left_min or has_matching_elements_left_max or has_matching_elements_right_min or has_matching_elements_right_max:
                        print("policy actions triggered robotic arm action filter")
                        #input("please input:")
                ...
                print("model_time", test_inference_time)
...
def get_arguments():
    ...
    parser.add_argument('--img_front_depth_topic', action='store', type=str, help='img_front_depth_topic',
                        default='/camera2/aligned_depth_to_color/image_raw', required=False)
    parser.add_argument('--img_left_depth_topic', action='store', type=str, help='img_left_depth_topic',
                        default='/camera1/aligned_depth_to_color/image_raw', required=False)
    parser.add_argument('--img_right_depth_topic', action='store', type=str, help='img_right_depth_topic',
                        default='/camera3/aligned_depth_to_color/image_raw', required=False)
    ...
    parser.add_argument('--use_action_protection', action='store', type=bool, help='use_action_protection',
                        default=False, required=False)
